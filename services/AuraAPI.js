...
/**
 * Fetch activities by kind from the Aura Brain API.
 *
 * @param {string} kind - The vibe kind (e.g., "Study Break", "Beach Vibes")
 * @returns {Promise<Array>} - Array of activity objects
 * @throws {Error}            - On network failure, timeout, or non-2xx response
 */
export async function fetchActivitiesByKind(kind) {
  const controller = new AbortController();
  const timerId = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);

  try {
    const token = await SecureStore.getItemAsync('discover_au_jwt');
    const response = await fetch(`${BASE_URL}/api/vibes/by-kind`, {
      method: 'POST',
      headers: { 
        'Content-Type': 'application/json',
        ...(token ? { 'Authorization': `Bearer ${token}` } : {})
