...
import { ArrowLeft, Check, Sparkles, Shield, AlertCircle } from 'lucide-react-native';
...
const SECURE_STORE_KEY = 'discover_au_personalized_recommendation';

export default function PersonalizedRecommendationSettingsScreen() {
  ...
  useEffect(() => {
    async function loadSetting() {
      try {
        ...
        console.log('[PersonalizedRecommendationSettingsScreen] Error loading local setting:', err);
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleSelect = async (newValue) => {
    ...
    try {
      ...
      if (userToken) {
        await authService.setPersonalizedRecommendation(userToken, newValue);
      }

      // 2. Persist locally
      await SecureStore.setItemAsync(SECURE_STORE_KEY, newValue ? 'true' : 'false');
      
      showToast('success', `Personalized recommendations ${newValue ? 'Enabled' : 'Disabled'}!`);
    } catch (err) {
      console.log('[PersonalizedRecommendationSettingsScreen] Error updating setting:', err);
    ...
    <SafeAreaView style={s.screen} edges={['top', 'bottom']}>
      ...
      >
        ...
        <View style={s.introWrap}>
          <Text style={s.title}>Personalized Recommendations</Text>
          <Text style={s.subtitle}>
            Control whether DiscoverAustralia uses your data to provide personalized content recommendations. This helps us suggest places, squads, and experiences that match your preferences.
          </Text>
        </View>
              ...
              >
                <View style={s.cardHeader}>
                  <View style={[s.iconBox, isEnabled && s.activeIconBox]}>
                    <Sparkles size={20} color={isEnabled ? colors.primary : colors.textSecondary} strokeWidth={2} />
                  </View>
                ...
                <Text style={s.cardDesc}>
                  Allow personalized recommendations based on your data. Get tailored suggestions for places, squads, and experiences that match your interests and preferences.
                </Text>
              </LinearGradient>
            </TouchableOpacity>
          </Animated.View>
              ...
              >
                <View style={s.cardHeader}>
                  <View style={[s.iconBox, !isEnabled && s.activeIconBoxErr]}>
                    <Shield size={20} color={!isEnabled ? '#EF4444' : colors.textSecondary} strokeWidth={2} />
                  </View>
                  <View style={s.cardHeaderMiddle}>
                ...
                <Text style={s.cardDesc}>
                  Disable personalized recommendations. Your data won't be used for content suggestions. You'll see generic recommendations instead.
                </Text>
              </LinearGradient>
            </TouchableOpacity>
          </Animated.View>
        ...
        <View style={s.infoBanner}>
          ...
          <Text style={s.infoText}>
            Your privacy matters. When enabled, we only use your activity data to improve your experience. You can change this setting anytime. All data is encrypted and secure.
          </Text>
        </View>
